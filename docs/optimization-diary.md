...
### #30 前端 JS 反复编辑导致重复定义/孤立代码块（语法错误全页挂掉）
- **坑**：给 index.html 加流式功能时多次 Edit，最后 script 里出现**两个 `loadReport` 定义
  + 一个被切断的 `investigate` 函数体**（孤立代码块）。浏览器所有 JS 函数变 undefined，
  整页卡在"加载中"，无控制台报错。
- **根因**：HTML 单文件用 Edit 增量改 JS，函数边界在多次替换中错位。
- **解决**：整个 `<script>` 块重写为干净版本（Write 全文件），去掉重复。
- **避坑**：**前端单文件反复 Edit 高风险**——涉及函数重组时直接重写整个文件，
  且改完用 `typeof fn` 在浏览器验证所有函数存在。

### #31 ReAct 在 mock 数据下不触发（无异常日志簇）
- **坑**：注入 LLM 后 ReAct（`_react_log_dig`）仍不触发——它需要 `abnormal`（error 级
  异常日志簇），但 mock 日志要么是噪音（heartbeat）要么是 warn，没有 error 簇。
- **根因**：mock 数据构造时没考虑"触发 ReAct 需要异常日志簇"这个前提。
- **解决**：直接调 `_react_log_dig` 验证机制（工具受限/max_iters/兜底都对），
  但承认**项目里没有数据能展示 ReAct 的智能结果**——真实数据（RE2 有 error 日志）
  才可能触发。
- **避坑**：**验证"机制存在"和"数据触发"是两回事**——要展示 Agent 能力，
  数据源必须构造到能触发该能力的前提条件。

### #32 真实 LLM 在有界循环里不收敛（倾向继续查而非下结论）
- **坑**：给 LLM 控制权（5_agent 决策循环）后，真实 DeepSeek 4 次循环全调 `query_logs`，
  不产出 final_answer，`used_llm=False` 落兜底。prompt 明确写了"证据足够立即 conclude"
  仍不收敛。
- **根因**：真实 LLM agent 的固有行为——在有界 ReAct 里倾向"多查一点"而非"下结论"，
  且会把指标名当日志关键词查（`checkout_error_rate` 查 3 次日志"无匹配"）。这是业界
  实证（LLM 易在低价值查询打转），不是实现 bug。
- **解决**：接受"LLM 有控制权但收敛质量待调优"。安全网设计（max_iters 封顶 + fallback
  conclude=True）保证 LLM 不收敛时确定性主线不受影响。prompt 已强化（明确"不要用指标名
  查日志"、"立即 conclude"），收敛质量是后续调参项。
- **避坑**：**"LLM 有控制权"不等于"LLM 会聪明收敛"**——两者要分开验收。控制权 = 图会走
  agent 循环、工具真实可调、失败兜底；收敛质量 = prompt/模型调优。别用"LLM 是否给出好
  结论"验收架构改造，用"控制循环是否运转 + 兜底是否接住"验收。
